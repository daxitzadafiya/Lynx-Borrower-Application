...
async function getTableData() {
  if (!store.state.isHosteCreditOrderCalled) {
    await store.dispatch("getCreditOrders", true);
    store.dispatch("setHostedModeOrdersStatus", true);
  }
  const { reportDetails, housingExpenses, currentLoan } = store.state;
