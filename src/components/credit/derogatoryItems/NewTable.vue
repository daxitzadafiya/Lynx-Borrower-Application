...
.d-contents
  v-data-table.content-table(
            ...
            template(v-slot:body.append v-if="derogatories!=null && derogatories.length!==0")
                tr.totals-row
                    td Total:
                    td
                    td
                    td
                    td 
                    td.text-center ${{ getFormattedValue(totalObligationAmount) }}
                    td
  v-dialog(
            v-model="isDialog"
            max-width="290"
...
export default {
  ...
  methods: {
    ...
    editItem(derogatoryItem) {
      ...
      this.toggleIsOnlay({
        onlayComponent: "editDerogatoryItem",
        isOnlay: true
      });
    },
