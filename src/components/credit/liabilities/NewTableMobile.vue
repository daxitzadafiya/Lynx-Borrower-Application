<template lang="pug">
  .d-contents
    .text-subtitle-1.mb-3.font-weight-bold If you have any mortgages, car payments, credit cards, or installment loans not listed, please add them using the Add New + button below. 
    ...
    v-row.totals-row
            v-col.add-new.d-flex
              v-btn(dense plain elevation="0" @click="addLiability").addItemBtn
                v-icon(small) mdi-plus
                span.addBtnText Add Other Liabilty
            v-col(sm="2" cols="4")
                v-row.totals-row_bold.totals-row_aligned-bottom Totals:
            v-col(sm="3" cols="4")
                v-row.text-block__row.data-title Unpaid Balance
                v-row.totals-row_bold.data-value ${{ getFormattedValue(totalUnpaid) }}
            v-col(sm="3" cols="4")
                v-row.data-title Monthly Payment
                v-row.totals-row_bold.data-value ${{ getFormattedValue(totalPayment) }}
</template>
